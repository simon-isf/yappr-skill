...
1. **Server mints a session** — `POST /calls {type:"web", agent_id}` with the secret API key (optionally `lead_id` and `metadata`). Returns `{ call_id, token, connection, protocol }` (see **POST /calls — web call session** in `yappr-api.md`). Nothing is dialled and the secret key stays on the server, but the call already exists: `GET /calls/{call_id}` reads `pending_connection` until the browser connects, and an unused session settles `no_answer` (`session_expired`, cost 0) as soon as it is read after the token expires (or at the next five-minute sweep) — so mint on click, not on page load. To take back a session nobody will use, `POST` the mint's `cancel_url` with the token as `x-yappr-web-token` and no body — on both protocols, and the only way on a `call_request` session; an `offer` session's call then ends as an operator's end (`ended_by: "operator"`, no `failure`, cost 0), exactly as `POST /calls/{id}/end` ends it with the secret key.
...
**Who bills what.** Telnyx bills the customer's own account for the phone minutes at their rates, Telnyx's own fees (call control, media streaming, recording, noise suppression) and the numbers themselves; none of it appears on the Yappr bill. Yappr bills the agent minutes at its Phone calls price (Billing → Prices), the same as a call from a number bought from Yappr — not lowered for the phone minutes Telnyx bills them. One exception: a call with a Palestinian Authority number at either end pays a premium price on a Yappr number, and the plain Phone calls price here. Say this before they connect — it is the first question they will have.

**0. Give the key the carrier scopes.** Carrier accounts are available to every workspace — nothing to switch on. The scopes are opt-in, though: no key has them by default. In Settings → API keys, create a key with the **Your carrier (Telnyx)** group ticked (`carrier_accounts:read` and `carrier_accounts:manage`); without them every carrier route but one answers `403 INSUFFICIENT_SCOPE` naming the scope. The one is `GET /carrier-accounts/status`: it needs no scope, so any key for the workspace (not one limited to some agents) can ask whether the feature is on. That is the key, not the feature: widen the key's scopes, don't rotate it. (A `403` with the code `CARRIER_ACCOUNTS_NOT_ENABLED` means Yappr switched the feature off for this workspace in an emergency — contact Yappr support. Branch on `code`, never on the status alone.)
