...
## Part 11: Conversation Arc — When to Discover, When to Transact

The previous parts covered *micro-mechanics* — turn-taking, listening signals, threading. This part covers the *macro shape* of the whole call. It's the failure mode reviewers most often describe as: "the agent answered correctly, but the call felt transactional. It didn't feel human."

### The 5-phase arc

Every well-run human call (sales, support, reception, qualification) has roughly the same structure:

```
1. GREETING       (1 turn)        warm, brand, name
2. ACKNOWLEDGE    (1 turn)        receive their reason, mirror it
3. DISCOVER       (1–3 turns)     show genuine curiosity about their situation
4. RECOMMEND      (1 turn)        propose the next step
5. TRANSACT       (variable)      collect what's needed, confirm
6. CLOSE          (1 turn)        warm exit
```

Untrained agents (and naively-prompted LLMs) collapse phases 2–3 into "intent capture" and skip 4 entirely, going straight from greeting to phase 5. **This is the single biggest reason an agent feels robotic even when its individual responses are well-formed.** Listeners can't articulate it, but they feel it: "the bot didn't really care why I called, it just wanted to schedule me."

### Discovery is what makes phase 5 feel earned

A receptionist who hears "I want Invisalign" and replies "What's your name and phone?" comes across as a scheduling kiosk. A receptionist who hears the same thing and replies "Sure — was this something you've been thinking about for a while, or did something specific bring you in today?" comes across as a person. Same outcome (booked appointment), entirely different perceived warmth.

The minimum bar: **one discovery question, even on calls that seem fully transactional**. Discovery isn't qualification — it's curiosity. The aim isn't to gather data, it's to make the caller feel seen.

### Discovery question types (use one, not all)

| Type | Example | When |
|---|---|---|
| **Backstory** | "How long has this been on your mind?" | Considered purchases (cosmetic, elective, expensive) |
| **Trigger** | "Did something specific bring you in today?" | When the timing isn't obvious |
| **Source** | "How did you hear about us?" | Lead-attribution worth + warm conversation starter |
| **Prior experience** | "Have you done X before, or is this your first time?" | Anything with a learning curve |
| **Concern depth** | "What's the part that worries you most?" | Health, legal, financial — anything personal |

Pick one per call. **Two discovery questions is interrogation. Zero discovery questions is robotic.**

### When to skip discovery

| Caller posture | What to do |
|---|---|
| Urgent / pain / emergency | **Skip** — go straight to recommend. Discovery here is callous. |
| Clipped, time-pressed, "just give me the slot" | **Skip or shorten to one acknowledgment** — match their energy. |
| Already-known customer in transactional flow (cancel/reschedule) | **Skip** — they want efficiency, not a chat. |
| Existing patient calling about a routine appointment | **Light** — one warm sentence ("good to hear from you, how have you been?") rather than a probing question. |
| New lead with a considered intent (Invisalign, implants, whitening, big purchase) | **Required** — at least one discovery question before any data collection. |
| Cold call / outbound | **Required** — one discovery beat is what separates a sales call from a robocall. |
| Information request | **Skip** — answer the question. A "how did you hear about us?" before a "what are your hours?" is annoying. |

The agent's job is to **read the room**. If the caller is rushing, match their pace. If they're chatty, expand. Don't apply discovery dogmatically — apply it as a default with explicit exceptions.

### The discovery transition (phase 3 → phase 4)

Discovery without a graceful transition feels like a survey. The bridge to recommend should pick up something specific they said:

> Caller: "I've had crooked teeth my whole life, finally have the budget."  
> Agent: "Got it — twenty years on, you decided this is the moment. **Look,** Invisalign consult here is free, includes a digital scan so you walk out with a real picture of how it'd look. Want to grab a slot this week?"

The "Look," / "OK," / "תראו —" / "אז ככה —" is the prosodic shift from listening to recommending. It signals: "I heard you, now I'm being helpful."

### How to encode this in a flow agent

For a flow agent, encode the arc *inside* each conversation node's instructions. Don't make discovery a separate node — that fragments the arc and makes the model treat it as a distinct phase to "complete and move on" instead of a tonal beat woven into the conversation.

Each conversation node that handles a "considered" intent (booking, sales) should structure its instructions like:

```
Goal: <outcome of this node>

Required arc (in order, can shorten if caller is time-pressed):
1. Acknowledge — mirror what they said in one short sentence.
2. Discover — ask ONE open question about their situation.
3. Recommend — propose the next step / explain the offer.
4. Transact — collect what's missing only.
5. Confirm.

Skip-discovery rule:
If caller signals urgency, time-pressure, or "just give me the slot,"
match their pace and go acknowledge → recommend → transact.
```

Nodes that handle inherently transactional intents (cancel, reschedule, billing-question, emergency, info request) **omit phase 2–3** explicitly:

```
Goal: <transactional outcome>
Skip discovery — this is a transactional call. Go straight to acknowledge → transact.
```

### What scripted feels like vs what discovery feels like

```
SCRIPTED — collapsed arc                    GOOD — full arc
────────────────────────                    ────────────────
"Thank you for calling.                     "Smile Up, this is Shani."
 I'd be happy to help."
"Can I get your name?"                      "Hey, sure — Invisalign? Walk me
"Phone number?"                              through it, what's been on
"Preferred date?"                            your mind?"
"Confirmed for the 14th. Bye."              [caller shares context]
                                            "Got it. Look, the consult here
                                             is free, includes the scan.
                                             Tomorrow at 3 work?"
                                            [caller confirms]
                                            "Done. Dana, I have you down
                                             for tomorrow at 3. SMS in a
                                             minute."
```

Same data collected. One sounds like a kiosk. The other sounds like a person you'd recommend to a friend.

### The wrong way to add discovery

A common over-correction: forcing discovery on every call regardless of context. The agent that asks "What brought you in today?" to someone calling to cancel an appointment is worse than no discovery at all — it's tone-deaf. The discovery posture is **default-on with explicit skip-rules**, never blanket-applied.

Another wrong way: treating discovery as data collection. "How did you hear about us?" asked because you want the marketing attribution data is qualification, not curiosity. The model can tell the difference, and the caller can hear it. Frame discovery in the prompt as *connection*, not *intake*.

---

## Part 12: The Uncanny Valley
